...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<V> adjVertices(V vert) {
        if(!validVertex(vert))
            return null;
        MapVertex<V,E> adjVertex = mapVertices.get(vert);

        return  adjVertex.getAllAdjVerts();
    ...
    public Location getVertex(int x, int y) {
        Predicate<Location> predicate = (n) -> n.getX()== x && n.getY()== y;

        return (Location) vertex((Predicate<V>) (predicate = (n) -> n.getX()== x && n.getY()== y));
    }
}